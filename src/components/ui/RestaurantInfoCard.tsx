...
export default function RestaurantInfoCard({ restaurant, onClose }: RestaurantInfoCardProps) {
  ...
  const Content = () => {
    ...
    <>
      ...
      <button 
        onClick={onClose}
        className="absolute top-4 right-4 md:right-auto md:left-4 p-2 bg-black/20 hover:bg-black/40 md:bg-white/80 dark:md:bg-zinc-900/80 md:hover:bg-white dark:md:hover:bg-zinc-800 backdrop-blur-md rounded-full text-white md:text-gray-900 dark:md:text-zinc-100 shadow-sm md:shadow-md transition-all z-40 group"
      >
      ...
      <div className="overflow-y-auto hide-scrollbar pb-8 flex-1">
        ...
        <div className="p-6 md:p-8">
          ...
          {/* 식당 타이틀 & 카테고리 */}
          <h3 className="text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-zinc-100 mb-1.5 tracking-tight leading-tight">
            {restaurant.name}
          </h3>
          <div className="flex items-center text-sm text-gray-500 dark:text-zinc-400 font-medium mb-6">
          ...
          {/* 유튜브 한줄평 코멘트 박스 */}
          {restaurant.videos && restaurant.videos.length > 0 && (
            <div className="mb-8 bg-red-50/70 dark:bg-red-950/15 rounded-2xl p-4 md:p-5 border border-red-100 dark:border-red-900/30 flex gap-3 items-start shadow-sm relative overflow-hidden">
              ...
              <img 
                ...
                className="w-10 h-10 md:w-12 md:h-12 rounded-full border-2 border-white dark:border-zinc-800 shadow-md z-10" 
                alt="profile"
              />
              <div className="z-10">
                <span className="text-xs md:text-sm font-bold text-red-600 dark:text-red-400 mb-1 block">
                  {restaurant.videos[0].youtuber.name}의 한줄평
                </span>
                <p className="text-[15px] md:text-base text-gray-900 dark:text-zinc-200 leading-snug font-semibold">
                  {restaurant.videos[0].is_short 
          ...
          {/* 주소 정보 */}
          <div className="flex items-start text-[15px] text-gray-700 dark:text-zinc-300 mb-8 font-medium bg-gray-50 dark:bg-zinc-900/40 p-3 md:p-4 rounded-xl border border-gray-100 dark:border-zinc-800/40">
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="flex-1 flex items-center justify-center gap-2 bg-gray-900 dark:bg-zinc-100 text-white dark:text-zinc-950 py-3.5 px-4 rounded-xl text-[15px] font-bold hover:bg-gray-800 dark:hover:bg-zinc-200 transition-all hover:shadow-lg active:scale-[0.98]"
            >
              카카오맵 상세보기
            ...
            <button 
              ...
              className="flex-1 flex items-center justify-center bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 border-2 border-gray-200 dark:border-zinc-800 py-3.5 px-4 rounded-xl text-[15px] font-bold hover:bg-gray-50 dark:hover:bg-zinc-800 transition-all active:scale-[0.98]"
            >
              길찾기
            </button>
          </div>
      ...
      {restaurant && (
        <>
          ...
          <motion.div
            ...
            className="md:hidden absolute bottom-0 left-1/2 -translate-x-1/2 z-30 w-full max-w-md bg-white dark:bg-zinc-950 rounded-t-[32px] shadow-[0_-10px_50px_rgba(0,0,0,0.2)] overflow-hidden flex flex-col max-h-[85vh]"
          >
            <Content />
          </motion.div>
          ...
          <motion.div
            ...
            className="hidden md:flex absolute top-6 left-[412px] bottom-6 w-[380px] z-30 flex-col bg-white/95 dark:bg-zinc-950/85 backdrop-blur-2xl rounded-[28px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.2)] border border-white/60 dark:border-white/10 overflow-hidden"
          >
            <Content />
          </motion.div>
        </>
